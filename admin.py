...
# Auxiliary functions
###########################
def process_lines(lines, hei, officer, host, file=True):
    # Loads lines from a list of dictionaries, usually from a CSV file,
    # but it could also come from the simulated CSV objects for batch loading.
    # This was requested by very small institutions with a reduced number of
    # mobile students.
    # Line numbering depends on the origin of the processed lines
    total = 1 if file else 0
    cards = 0
    deleted = 0
    new_persons = []
    new_cards = []
    errors = []
    # The load operation has to succeed for the batch
        ...
        try:
            for line in lines:
                ...
                if (
                    esi in (None, '')
                    or email in (None, '')
                    or first_name in (None, '')
                    or last_name in (None, '')
                ):
                    errors_in_line = True
                    total += 1
                    errors.append(_('Missing data on line {0}').format(total))
                opcode = line.get('operation', 'C')
                if opcode is None or opcode.strip() == '':
                    opcode = 'C'
                if opcode not in ['C', 'D']:
                    error = True
                    total += 1
                    errors.append(
                        _('Bad operation {0} on line {1}').format(
                            opcode, total
                        )
                    )
                if errors_in_line:
                ...
                if opcode == 'C':
                    ...
                    if newcard and not new:
                        new_cards.append(card.pk)
                    total += 1
                if opcode == 'D':
                    # There MUST be one card per person per hei and esi
                    card = StudentCard.objects.filter(
                        hei=hei, esi=esi.strip()
                    ).first()
                    if card is None:
                        errors.append(
                            _('Card not found on line {0}').format(total)
                        )
                        continue
                    card.delete()
                    deleted += 1
...
class HEIAdmin(admin.ModelAdmin):
    ...
    def has_view_permission(self, request, obj=None):
        ...
        if obj is not None and (
            request.user.id == obj.managedBy.id or request.user.is_superuser
        ):
            return True
        return False
    ...
    def get_queryset(self, request):
        if request.user.is_superuser:
            return HEI.objects.all()
        if request.user.is_officer:
            return request.user.HEIs
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        fieldsets = [
            ...
            (
                _('Erasmus'),
                {'fields': [('oid', 'euc'), ('erc', 'sho', 'pic')]},
            ),
            (
                _('European Student Card'),
                {'fields': ['productionKey', 'sandboxKey']},
            ),
        ]
    ...
    def get_urls(self):
        urls = super().get_urls()
        urls.insert(
            0, path('hei_load/', self.admin_site.admin_view(self.load_heis))
        )
        return urls
    ...
    def load_heis(self, request):
        ...
        if request.method == 'POST':
            ...
            lines = csv.DictReader(
                io.StringIO(data.read().decode('utf-8-sig')),
                delimiter=delimiter,
            )
            ...
            for line in lines:
                ...
                for field in ['name', 'country', 'manager', 'sho']:
                    if line.get(field, '').strip() == '':
                        errors_in_line = True
                        errors.append(
                            _('Field {0} missing on line {1}').format(
                                field, total
                            )
                        )
                if errors_in_line:
                ...
                if manager is None:
                    ...
                    if first_name == '' or last_name == '':
                        e = _(
                            'Person {0} in line {1} does not exist. '
                ...
                hei.termstart = line.get(
                    'termstart', get_setting('TERM_START', 9)
                ).strip()
                try:
                    hei.save()
                    inserted += 1
                    ...
                    officer, n = Officer.objects.get_or_create(
                        hei=hei, person=manager, manager=user
                    )
                    # Add manager to the list of invites
                    if new:
                        new_persons.append((manager, hei))
            ...
            if not inserted == 0:
                self.message_user(
                    request,
                    ngettext(
                        '{0} HEI loaded', '{0} HEIs loaded', inserted
                    ).format(inserted),
                    messages.SUCCESS,
                )
            ...
            if not newpersons == 0:
                self.message_user(
                    request,
                    ngettext(
                        '{0} person created', '{0} persons created', newpersons
                    ).format(newpersons),
                    messages.SUCCESS,
                )
            # We can send the invites to those that have just been added
            ...
            for person in new_persons:
                ...
                if not result:
                    errors.append(
                        _('Message to {0} not sent').format(person[0].email)
                    )
...
class OfficerAdmin(admin.ModelAdmin):
    ...
    def has_view_permission(self, request, obj=None):
        ...
        if obj is None:
            return True
        if obj is not None and (
            request.user.id == obj.manager.id or request.user.is_superuser
        ):
            return True
        return False
    ...
    def get_urls(self):
        ...
        urls.insert(
            0,
            path(
                'officer_load/', self.admin_site.admin_view(self.load_officers)
            ),
        )
        return urls
    ...
    def load_officers(self, request):
        ...
        if request.method == 'POST':
            ...
            for line in lines:
                ...
                new = not Officer.objects.filter(
                    hei=hei, person=person
                ).exists()
            ...
            if not inserted == 0:
                self.message_user(
                    request,
                    ngettext(
                        '{0} officer loaded', '{0} officers loaded', inserted
                    ).format(inserted),
                    messages.SUCCESS,
                )
            # Person objects created
        ...
        if len(errors) == 0:
            ...
            if len(user.HEIs) > 1:
                ...
                form.fields['hei'] = forms.ChoiceField(
                    label=_('Select HEI'), initial=user.myHEI, choices=choices
                )
        context = dict(self.admin_site.each_context(request), form=form)
...
class StudentCardAdmin(admin.ModelAdmin):
    ...
    select_related = True
    fieldsets = [
        (None, {'fields': [('student', 'hei'), ('esi', 'esc'), 'manager']})
    ]
    readonly_fields = ['esc']
    ...
    def has_add_permission(self, request):
        ...
        if request.user.is_superuser or request.user.is_officer:
            return True
        if super(StudentCardAdmin, self).has_add_permission(request):
            return True
        return False
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'student':
            kwargs['queryset'] = request.user.persons
        return super().formfield_for_foreignkey(db_field, request, **kwargs)

    def get_queryset(self, request):
        # Only managed cards will be shown
        return StudentCard.objects.filter(
            manager__in=request.user.manages.all()
        )
    ...
    def get_urls(self):
        urls = super().get_urls()
        urls.insert(
            0, path('card_load/', self.admin_site.admin_view(self.load_cards))
        )
        return urls
    ...
    def export_cards(self, request, queryset):
        ...
        for card in queryset.all():
            writer.writerow(
                [card.__getattribute__(field) for field in fields_names]
            )
        return response
